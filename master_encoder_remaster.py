...
def encode_grayscale_lix(image_path):
    ...
    # Method 2: Delta + Huffman
    huff = huffman_encode(delta)
    methods[2] = huff
...
def compress_rgb_with_hybrid(image_array):
    ...
    flat_delta = delta.flatten().tobytes()
    compressed = brotli.compress(flat_delta)
    return compressed
...
def save_lix_image(image_path, output_path):
    ...
    if mode == "L":
        # Grayscale (unchanged except removing bit-packing)
        header = b"LIXG"
        data = np.array(img).flatten()
        delta = delta_encode(data)
        delta_rle = rle_encode(delta)
        ...
        # RGB (remove bit-packing)
        img = img.convert("RGB")
        arr = np.array(img).reshape(-1, 3)
        compressed = compress_rgb_with_hybrid(arr)
        header = b"LIXF" + struct.pack(">HHBB", width, height, 0x03, 3)
        with open(output_path, "wb") as f:
            f.write(header + compressed)
        print(f"✅ Saved .lix RGB: {output_path}, size: {len(compressed)} bytes")
    
    elif mode == "RGBA":
        # RGBA (ensure lossless compression for both RGB and alpha)
